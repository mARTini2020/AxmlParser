...
static AxmlEvent_t axmlNextEvent = -1;
...
AxmlNext(void *axml)
{
	...
	if(axmlNextEvent == -1)
	{
		axmlNextEvent = AE_STARTDOC;
		return axmlNextEvent;
	}
	...
	/* when buffer ends */
	if(NoMoreData(ap))
		axmlNextEvent = AE_ENDDOC;

	if(axmlNextEvent == AE_ENDDOC)
		return axmlNextEvent;
	...
	else if(chunkType == CHUNK_TEXT)
	{
		...
		axmlNextEvent = AE_TEXT;
	}
	else
	{
		axmlNextEvent = AE_ERROR;
	}
